...
size_t _heap_get_elem_level(heap_t *heap, void *elem) {
    ...
    ptrdiff_t diff = (intptr_t)elem - (intptr_t)heap->data;
    size_t idx = diff / heap->elem_size;
    return 63 - _lzcnt_u64(idx);
}

void *_heap_get_level_ptr(heap_t *heap, size_t level){
    return &((char *)heap->data)[(1 << level) * heap->elem_size];
}
...
void _heap_heapify_down(heap_t *heap, void *cur){
    ...
    // printf("%d\n",*(int*)down_left);
    int cmp = heap->comparator(down_left,cur);
    if(cmp >= 0){
        _heap_swap(heap,cur,down_left);
        _heap_heapify_down(heap,down_left);
    }
    cmp = heap->comparator(down_right,cur);
    if(cmp >= 0){
        _heap_swap(heap,cur,down_right);
        _heap_heapify_down(heap,down_right);
    }
}
...
void heap_insert(heap_t *heap, void *data) {
    ...
    size_t level,index;
    void *next_level;
    _heap_get_elem_level_index(heap,heap->head,&level,&index);
    next_level = _heap_get_loc_down(heap,heap->head);
    printf("%lu %lu %lu %lu\n", level, index, heap->head, next_level);
...
void *heap_extract(heap_t *heap) {
    ...
    void *next_level;
    _heap_get_elem_level_index(heap,heap->head,&level,&index);
    next_level = _heap_get_loc_up(heap,heap->head);
    printf("%lu %lu %lu %lu\n", level, index, heap->head, next_level);
    

    void *data = malloc(heap->elem_size);
    memcpy(data, heap->data, heap->elem_size);
    memcpy(heap->data, &((char *)heap->head)[-heap->elem_size], heap->elem_size);
